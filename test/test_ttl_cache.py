...
class TestTTLEvictionPolicy(unittest.TestCase):
    ...
    def test_ttl_expiration(self):
        ...
        ttl_cache.put("a", 1)
        time.sleep(0.3)
        self.assertEqual(ttl_cache.get("a"), 1, msg="Failed to retrieve value '1' for key 'a' before expiration")

        time.sleep(0.3)
        self.assertIsNone(ttl_cache.get("a"), msg="Expected None after expiration of key 'a'")
    ...
    def test_ttl_eviction_after_insert(self):
        ...
        ttl_cache.put("c", 3)
        self.assertIsNone(ttl_cache.get("a"), msg="Key 'a' should have expired and be None")
        self.assertIsNone(ttl_cache.get("b"), msg="Key 'b' should have expired and be None")
        self.assertEqual(ttl_cache.get("c"), 3, msg="Failed to retrieve value '3' for key 'c'")
    ...
    def test_ttl_retains_valid_entries(self):
        ...
        ttl_cache.put("b", 2)
        time.sleep(0.3)
        ttl_cache.put("c", 3)  # Should evict "a" as "b" is more recently used and not expired

        self.assertIsNone(ttl_cache.get("a"), msg="Key 'a' should have been evicted")
        self.assertEqual(ttl_cache.get("b"), 2, msg="Key 'b' should be retained as it is still valid")
        self.assertEqual(ttl_cache.get("c"), 3, msg="Failed to retrieve value '3' for key 'c'")
    ...
    def test_ttl_expiration_for_recently_accessed(self):
        ...
        time.sleep(0.3)
        self.assertIsNone(ttl_cache.get("a"), msg="Key 'a' should not have expired as it was accessed recently")
    ...
    def test_ttl_mixed_operations(self):
        ...
        time.sleep(0.3)
        self.assertIsNone(ttl_cache.get("a"), msg="Key 'a' should have expired after 0.4 seconds")
        ...
        self.assertEqual(ttl_cache.get("b"), 2, msg="Key 'b' should still be available after adding 'd'")
        self.assertEqual(ttl_cache.get("d"), 4, msg="Failed to retrieve value '4' for key 'd'")
